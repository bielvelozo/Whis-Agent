 ...
 * re-converta o resultado do bold (`*x*` virando `_x_`). Placeholders
 * usam tokens ASCII improváveis em conversa natural; colisão é aceita
 * como risco residual (mensagem pessoal no WhatsApp não vai conter
 * `__WHIS_C_42__` literal).
 */
export function toWhatsAppText(input: string): string {
  ...
  s = s.replace(/__WHIS_B_(\d+)__/g, (_, idx: string) => `*${boldContents[Number(idx)]}*`);

  // Stage 5: restaurar code blocks/inline.
  s = s.replace(/__WHIS_C_(\d+)__/g, (_, idx: string) => codePlaceholders[Number(idx)] ?? '');
